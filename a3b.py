...
MODEL = "a3b_"+sys.argv[1]+"_"+hour+'_'+minute
...
#model 2: NN with hidden layer of size HIDDEN_DIM	
def q_hat(state):
	w1 = tf.get_variable("weight1", shape=[STATE_DIM, HIDDEN_DIM], dtype=tf.float64, initializer=tf.contrib.layers.xavier_initializer())
	w2 = tf.get_variable("weight2", shape=[HIDDEN_DIM, ACTION_DIM], dtype=tf.float64, initializer=tf.contrib.layers.xavier_initializer())
